defmodule Ollo.GrantTypes.DeviceFlowTest do
  ...
  describe ".get_temp_code_info/1 on device_flow with a valid user_code" do
    setup do
  ...
  describe ".get_temp_code_info/1 on device_flow with a invalid user_code" do
  ...
  describe ".get_tokens/1 with granted device_code" do
    ...
    test "returns invalid_code after first fetch", %{client_id: client_id, code: code, user_code: user_code} do
      {:ok, %{tokens: tokens}} = Ollo.GrantTypes.DeviceFlow.get_tokens(%{code: code, client_id: client_id})
